  ...
  return (
    <div className="min-h-screen bg-[#0D0B1A] bg-grid">
      <div className="max-w-4xl mx-auto px-8 py-12">
          ...
          <div className="prose prose-invert max-w-none space-y-10">
            {sections.map((s, i) => (
              <div key={i} className="animate-fadeIn" style={{ animationDelay: `${i * 80}ms` }}>
                <h2 className="text-[18px] font-semibold text-white mb-3">{s.title}</h2>
                {s.content.split("\n").map((line, j) => (
                  <p key={j} className="text-[14px] text-[#94A3B8] leading-relaxed mb-2 last:mb-0 whitespace-pre-line">{line}</p>
                ))}
              </div>
            ))}
          </div>
        ...
        <div className="text-center mt-8 text-[12px] text-[#94A3B8] space-y-1">
